...
class NinjaBackend:
    ...
    def _write_ninja(self) -> None:
        ...
        python_command = (
            ...
            if sys.platform == "win32" else shlex.quote(sys.executable)
        ).replace("$", "$$")
        lines = [
            ...
            "",
            "rule ar_rule",
            # ar replaces supplied members but keeps omitted ones. Recreate
            # the archive when Ninja rebuilds it so removed sources cannot
            # survive as stale code. Python also works on Windows, unlike rm.
            f'  command = {python_command} -c "from pathlib import Path; import subprocess, sys; '
            'Path(sys.argv[1]).unlink(missing_ok=True); '
            'sys.exit(subprocess.call(sys.argv[2:]))" $out $ar rcs $out $in',
            "  description = AR $out",
            "",
